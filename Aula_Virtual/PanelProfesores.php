				...
				<div class="card text-white  mb-3" style="background-color:blue;max-width: 18rem;min-height:20rem;">
					<div class="card-header">Mis Temarios</div>
						<div class="card-body "style="background-color:#87CEFA;">
							<p class="card-text " >
							<?php
								$VerMaterias="select distinct M.Nombre,t.MateriaId,m.Nombre ,n.Denominacion,n.NivelId  from temas t,niveles n,profesores p,materias m where (t.NivelId =n.NivelId and p.ProfesorId =t.ProfesorId and m.EsActiva =true and P.ProfesorId=$Id_Actual)";
								$Temario=mysqli_query($conexion,$VerMaterias);
								
								echo("<div>");
								while($fila=mysqli_fetch_array($Temario)){
									echo("<p style='color:black;'><a href='VerTema.php?Id_Activo=".$fila["MateriaId"]."'style='color:black;'>Tema:".$fila["Nombre"]."</p></a><p style='color:black;'>".$fila["Denominacion"]."</p>");
